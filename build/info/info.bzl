...
# The script used by the rule below uses certain files generated by Jiri
# hooks when they are available, or falls back to calling `git` otherwise.
#
# Since Bazel doesn't support missing input files, use the values
# of 'available_jiri_hook_files' to determine the rule's proper attributes,
# and later, how to invoke the script.
_USE_JIRI_HOOK_FILES = (
    # LINT.IfChange
    available_jiri_hook_files["build/info/jiri_generated/integration_commit_hash.txt"] and
    available_jiri_hook_files["build/info/jiri_generated/integration_commit_stamp.txt"]
    # LINT.ThenChange(//build/bazel/repository_rules.bzl)
)

_JIRI_HOOK_FILES_ATTRIBUTES = {
...
def _gen_latest_date_and_timestamp_impl(ctx):
    ...
    if _USE_JIRI_HOOK_FILES:
        cmd_args.append("--use-jiri-values")
...
gen_latest_date_and_timestamp = rule(
    ...
    } | _JIRI_HOOK_FILES_ATTRIBUTES | PY_TOOLCHAIN_DEPS,
)
