...
def _fuchsia_board_input_bundle_test_impl(ctx):
    board_input_bundle_config = ctx.attr.board_input_bundle[FuchsiaBoardInputBundleInfo].config
    golden_file = ctx.file.golden_file
    return [create_validation_script_provider(ctx, board_input_bundle_config, golden_file)]
