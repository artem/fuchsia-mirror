...
async fn play_device(request: fac::PlayerPlayRequest) -> Result<fac::PlayerPlayResponse, Error> {
            ...
            fac::PlayerRequest::Play { payload, responder } => {
                let response = match payload.destination {
                    ...
                    Some(fac::PlayDestination::DeviceRingBuffer(..)) => {
                        play_device(payload).await.map_err(|e| {
        ...
        match request {
            fac::RecorderRequest::Record { payload, responder } => {
                let record_result = match payload.source {
                    ...
                    Some(fac::RecordSource::DeviceRingBuffer(..)) => record_device(payload).await,
                    Some(unknown_source) => {
