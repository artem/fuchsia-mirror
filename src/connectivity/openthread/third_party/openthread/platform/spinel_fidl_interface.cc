...
void SpinelFidlInterface::WriteToRxFrameBuffer(std::vector<uint8_t> vec) {
  ...
  if (mReceiveFrameBuffer.CanWrite(vec.size())) {
    ...
    otError SpinelFidlInterface::WaitForFrame(uint64_t aTimeoutUs) {
      ...
      size_t size = platformCallbackWaitForFrameFromRadio(sOtInstancePtr, buffer,
                                                          SPINEL_FRAME_MAX_SIZE, aTimeoutUs);
      ...
      if (vec.size() == 0) {
        ...
        otPlatLog(OT_LOG_LEVEL_DEBG, OT_LOG_REGION_PLATFORM,
                  "spinel-fidl: failed to receive frame");
    ...
    void SpinelFidlInterface::Process(const void *aContext) {
      ...
      uint8_t buffer[SPINEL_FRAME_MAX_SIZE];
      for (;;) {
        size_t size = platformCallbackFetchQueuedFrameFromRadio(sOtInstancePtr, buffer,
    ...
    otError SpinelFidlInterface::ResetConnection(void) {
      otPlatLog(OT_LOG_LEVEL_NOTE, OT_LOG_REGION_PLATFORM,
                "SpinelFidlInterface::ResetConnection()");
      return OT_ERROR_NONE;
    }
