...
use net_types::{
    ethernet::Mac,
    ip::{AddrSubnet, Ip, IpVersionMarker, Ipv4, Ipv4Addr, Ipv6},
...
use crate::{
    ...
    ip::{
        ...
        gmp::{
            igmp::{self, IgmpTimerId},
            mld::{self, MldTimerId},
            GmpDelayedReportTimerId,
        },
    },
...
fn test_igmp_enable_disable_integration() {
    ...
    let range = now..=(now + igmp::DEFAULT_UNSOLICITED_REPORT_INTERVAL);
    struct TestConfig {
        ip_enabled: bool,
        gmp_enabled: bool,
    }
...
fn test_mld_enable_disable_integration() {
    ...
    let range = now..=(now + mld::DEFAULT_UNSOLICITED_REPORT_INTERVAL);
    struct TestConfig {
        ip_enabled: bool,
        gmp_enabled: bool,
    }
