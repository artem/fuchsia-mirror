...
zx_status_t IovarTest::IovarGet(char* buf, uint32_t buf_len) {
  ...
  WithSimDevice([&](brcmfmac::SimDevice* device) {
    ...
    status = brcmf_send_cmd_to_firmware(sim->drvr, client_ifc_.iface_id_, BRCMF_C_GET_VAR, buf,
                                        buf_len, false);
  });
  return status;
}
...
zx_status_t IovarTest::IovarSet(char* buf, uint32_t buf_len) {
  ...
  WithSimDevice([&](brcmfmac::SimDevice* device) {
    brcmf_simdev* sim = device->GetSim();
    status = brcmf_send_cmd_to_firmware(sim->drvr, client_ifc_.iface_id_, BRCMF_C_SET_VAR, buf,
                                        buf_len, true);
  });
  return status;
}
