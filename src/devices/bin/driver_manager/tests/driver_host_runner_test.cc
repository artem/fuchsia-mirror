...
class DriverHostRunnerTest : public gtest::TestLoopFixture {
 protected:
  void SetUp() override { ASSERT_EQ(ZX_OK, dir_loop_.StartThread()); }
  ...
  async::Loop& dir_loop() { return dir_loop_; }
  driver_runner::TestRealm& realm() { return realm_; }

 private:
  async::Loop dir_loop_{&kAsyncLoopConfigNoAttachToCurrentThread};
