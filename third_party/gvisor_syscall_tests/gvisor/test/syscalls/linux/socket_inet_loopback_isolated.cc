...
TEST_P(SocketInetLoopbackIsolatedTest, TCPConnectionReuseAddrConflicts) {
  ...
  const uint16_t port = ASSERT_NO_ERRNO_AND_VALUE(AddrPort(listener.family(), listen_addr));

  // Create a first connection.
  FileDescriptor conn_fd1 =
      ASSERT_NO_ERRNO_AND_VALUE(Socket(connector.family(), SOCK_STREAM, IPPROTO_TCP));
  ASSERT_THAT(setsockopt(conn_fd1.get(), SOL_SOCKET, SO_REUSEADDR, &kSockOptOn, sizeof(kSockOptOn)),
              SyscallSucceeds());

  sockaddr_storage conn_addr = connector.addr;
  ASSERT_NO_ERRNO(SetAddrPort(connector.family(), &conn_addr, port));
  ASSERT_THAT(RetryEINTR(connect)(conn_fd1.get(), AsSockAddr(&conn_addr), connector.addr_len),
              SyscallSucceeds());
  sockaddr_storage conn_bound_addr;
  addrlen = sizeof(conn_bound_addr);
  ASSERT_THAT(getsockname(conn_fd1.get(), AsSockAddr(&conn_bound_addr), &addrlen),
              SyscallSucceeds());
  ASSERT_EQ(addrlen, connector.addr_len);

  // Create the second connection that is bind to the same local address as the
  // first.
  FileDescriptor conn_fd2 =
      ASSERT_NO_ERRNO_AND_VALUE(Socket(connector.family(), SOCK_STREAM, IPPROTO_TCP));
  ASSERT_THAT(setsockopt(conn_fd2.get(), SOL_SOCKET, SO_REUSEADDR, &kSockOptOn, sizeof(kSockOptOn)),
              SyscallSucceeds());
  // Bind should succeed.
  ASSERT_THAT(bind(conn_fd2.get(), AsSockAddr(&conn_bound_addr), addrlen), SyscallSucceeds());

  // Connect should fail.
  ASSERT_NO_ERRNO(SetAddrPort(connector.family(), &conn_addr, port));
  ASSERT_THAT(RetryEINTR(connect)(conn_fd2.get(), AsSockAddr(&conn_addr), connector.addr_len),
